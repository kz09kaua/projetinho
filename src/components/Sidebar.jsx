...
import { 
  ...
  FaPills, FaUserCheck, FaTachometerAlt
} from 'react-icons/fa';
...
const Sidebar = () => {
  ...
  const menuItems = [
    // Comuns a paciente e admin (alguns também para atendente)
    ...
    // Apenas admin
    { path: '/configuracoes', label: 'Configurações', icon: FaCog, roles: ['admin'] },
    { path: '/painel-administrador', label: 'Painel Admin', icon: FaClipboardList, roles: ['admin'] },
    ...
    { path: '/atendimento-paciente', label: 'Atender Paciente', icon: FaUserCheck, roles: ['atendente'] },
  ];
  ...
  return (
    <aside className="fixed left-0 top-16 h-[calc(100vh-64px)] w-64 bg-surface border-r border-outline-variant flex flex-col z-40">
      <div className="p-5 border-b border-outline-variant">
        <h2 className="text-lg font-black text-primary">Minha UBS</h2>
        <p className="text-xs text-on-surface-variant">Unidade de Saúde</p>
      </div>
      ...
      <div className="p-4 border-t border-outline-variant space-y-2">
        <div className="px-3 py-2 text-sm">
          <p className="font-medium text-on-surface">{user?.name || 'Usuário'}</p>
        ...
        <button
          ...
          className="flex items-center gap-2 w-full px-4 py-2 text-error hover:bg-error-container/20 rounded-lg transition"
        >
