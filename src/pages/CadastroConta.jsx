...
const CadastroConta = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'cpf') {
      ...
      // Limitar a 15 caracteres (incluindo formatação)
    ...
    if (errors[name] || (name === 'cpf' && !cpfValid) || (name === 'telefone' && !telefoneValid)) {
  ...
  const validateForm = () => {
    ...
    else if (!validarTelefone(formData.telefone)) newErrors.telefone = 'Telefone inválido';
    ...
    <div className="min-h-screen bg-[#fcfdfe] flex items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-6">
          ...
          <h2 className="text-3xl font-black text-[#0f172a] tracking-tight">Criar Conta</h2>
          <p className="text-slate-500 mt-2">Preencha os dados para acessar o sistema</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="col-span-2">
              <label className="block text-sm font-bold text-[#0f172a] mb-1">Nome Completo</label>
              <div className="relative">
                <HiUser className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="text"
                  name="nome"
                  value={formData.nome}
            ...
            {/* CPF */}
            <div>
              <label className="block text-sm font-bold text-[#0f172a] mb-1">CPF</label>
              <div className="relative">
                <HiIdentification className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="text"
                  name="cpf"
                  value={formData.cpf}
              ...
              {!cpfValid && <p className="text-red-500 text-xs mt-1">CPF inválido</p>}
            </div>

            {/* Telefone */}
            <div>
              <label className="block text-sm font-bold text-[#0f172a] mb-1">Telefone</label>
              <div className="relative">
                <HiPhone className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="tel"
                  name="telefone"
              ...
              {!telefoneValid && <p className="text-red-500 text-xs mt-1">Telefone inválido</p>}
            </div>

            {/* E-mail */}
            <div className="col-span-2">
              <label className="block text-sm font-bold text-[#0f172a] mb-1">E-mail</label>
              <div className="relative">
                <HiMail className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="email"
                  name="email"
            ...
            {/* Senha */}
            <div>
              <label className="block text-sm font-bold text-[#0f172a] mb-1">Senha</label>
              <div className="relative">
                <HiLockClosed className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="password"
                  name="senha"
            ...
            {/* Confirmar Senha */}
            <div>
              <label className="block text-sm font-bold text-[#0f172a] mb-1">Confirmar Senha</label>
              <div className="relative">
                <HiLockClosed className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="password"
