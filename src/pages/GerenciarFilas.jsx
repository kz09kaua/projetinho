...
const GerenciarFilas = () => {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-on-surface-variant">Esta área é restrita a atendentes.</p>
        </div>
      </div>
    );
  }
  ...
  const [filas, setFilas] = useState([
    ...
    { 
      ...
      pacientes: [
        { nome: 'Fernanda Rocha', cpf: '999.888.777-66', dataNasc: '30/04/1995' },
  ...
  const filtrarPacientes = (pacientes, termo) => {
    ...
    return pacientes.filter(p => 
      p.nome.toLowerCase().includes(lowerTermo) || 
  ...
  const chamarEspecifico = (especialidadeIndex, pacienteIndex, paciente) => {
    Swal.fire({
      ...
      cancelButtonText: 'Cancelar',
      preConfirm: () => {
        const justificativa = document.getElementById('justificativa').value || 'Não informada';
  ...
  const removerPaciente = (especialidadeIndex, pacienteIndex, paciente) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const novasFilas = [...filas];
        const removido = novasFilas[especialidadeIndex].pacientes.splice(pacienteIndex, 1)[0];
        setFilas(novasFilas);
        Swal.fire({
  ...
  const adicionarPaciente = async (especialidadeIndex) => {
    ...
    if (formValues) {
      const cpfExiste = filas[especialidadeIndex].pacientes.some(p => p.cpf === formValues.cpf);
      if (cpfExiste) {
        Swal.fire('Erro', 'Este CPF já está na fila desta especialidade.', 'error');
        return;
      }
      const novasFilas = [...filas];
      ...
      setFilas(novasFilas);
      Swal.fire('Adicionado!', `${formValues.nome} foi adicionado à fila.`, 'success');
    }
  };

  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <h1 className="text-3xl font-black text-on-surface mb-2">Gerenciar Filas de Atendimento</h1>
      <p className="text-on-surface-variant mb-8">Chame o próximo paciente, adicione manualmente ou pesquise por nome/CPF.</p>

      <div className="grid md:grid-cols-3 gap-6">
        {filas.map((fila, idx) => {
          const pacientesFiltrados = filtrarPacientes(fila.pacientes, searchTerms[idx]);
          return (
            <div key={fila.id} className="bg-surface rounded-2xl border p-6 shadow-sm flex flex-col">
              ...
              <div className="mb-4 flex-1">
                <p className="font-bold">Pacientes na fila ({pacientesFiltrados.length})</p>
                {pacientesFiltrados.length === 0 ? (
                  ...
                  <ul className="mt-2 space-y-2 max-h-96 overflow-y-auto">
                    {pacientesFiltrados.map((p, i) => {
                      ...
                      const originalIndex = fila.pacientes.findIndex(pac => pac.cpf === p.cpf);
                      return (
                        <li key={i} className="text-sm p-3 bg-surface-container-high rounded-lg">
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="font-medium text-on-surface">{p.nome}</div>
                              <div className="text-xs text-on-surface-variant">CPF: {p.cpf}</div>
                              <div className="text-xs text-on-surface-variant">Nasc: {p.dataNasc}</div>
                            </div>
                            <div className="flex gap-2">
                              <button
                                onClick={() => chamarEspecifico(idx, originalIndex, p)}
                              ...
                              <button
                                onClick={() => removerPaciente(idx, originalIndex, p)}
        ...
        <button
          onClick={() => {
            if (logRemocoes.length === 0) {
              Swal.fire('Nenhum registro', 'Nenhuma ação registrada ainda.', 'info');
            } else {
              const logText = logRemocoes.map(l => 
